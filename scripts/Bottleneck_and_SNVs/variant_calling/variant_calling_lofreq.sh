#!/bin/bash
# conda activate ngs
mkdir ~/work/2020-09-01_COVID_NGS_pipeline/tmp2_lofreq
cd ~/work/2020-09-01_COVID_NGS_pipeline/tmp2_lofreq
...
do			
	...
	do
		...
		if [[ -f "$bamfile" ]] && ! [[ `find $bamfile -mmin -1` ]]; then
			...
			# mv data to achive
			mv $bamfile ../archive_bam/$bamfile
			mv $bam ../archive_bam/$bam
			# mv $bam".bai" ../archive_bam/
